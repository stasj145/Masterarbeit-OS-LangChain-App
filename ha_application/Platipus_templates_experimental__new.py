...
RESPONSE: {{"destination": "DEVICE_COMMAND", "next_inputs": "set the lights in the example room to 50%"}}
...
DEVICE ALIAS LIST (alias: device_id):
{device_list}

REMEMBER: There might be grammatical errors in the device_id, make sure to copy them exactly including any errors.
...
INQUIRY_PROMT_TEMPLATE = """### Instruction:    
...
JSON STRUCTURE: {{"type": "inquiry", "commands": [{{"device": device_id, "attribute": attribute}}]}}
...
REMEMBER: Use the exact device Ids from the "DEVICE ALIAS LIST" or from "HISTORY". There might be grammatical errors in the Ids, so make sure to copy the Ids exactly including any errors.
...
You are a helpfull and friendly assistant connected to a smart home system. Under "SMART HOME SYSTEM:" you are given information from the smart home system. You should create a 1 to 2 sentence reply telling the user what the smart home did.
...
You are a helpful and friendly AI assistant connected to a smart home system. Your name is {AI_NAME} The user has just said something has nothing to do with the smart home system itself.
